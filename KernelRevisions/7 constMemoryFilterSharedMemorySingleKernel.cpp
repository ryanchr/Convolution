                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  __shared__ uchar4 temp[(BLOCK_SIZE+8)*(BLOCK_SIZE+8)];  //£¨16+8£©*£¨16+8£©
  ...
  //   }
     ...
     for(int offset_y = 0; offset_y<4; offset_y++)
       {
         ...
         int ima_ra = min(max(ab_image_position_y-4+offset_y, 0), (numRows - 1));
       
         temp[(loc_image_position_y+offset_y)*(blockDim.x+8)+loc_image_position_x+4] = 
   ...
   //bottom    
   if(loc_image_position_y == blockDim.y-1)
     for(int offset_y = 4+1; offset_y<8+1; offset_y++)
       {
	     int ima_cb = min(max(ab_image_position_x, 0), (numCols - 1));
         int ima_rb = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
   ...
   //left     
   if(loc_image_position_x == 0)
     for(int offset_x = 0; offset_x<4; offset_x++)
       {
	     int ima_cc = min(max(ab_image_position_x-4+offset_x, 0), (numCols - 1));
         int ima_rc = min(max(ab_image_position_y, 0), (numRows - 1));
       
         temp[(loc_image_position_y+4)*(blockDim.x+8)+loc_image_position_x+offset_x] = 
         //   ((ab_image_position_y-4+loc_image_position_y < 0)
         //    ? inputChannel[ab_image_position_x] : 
   ...
   //right
   if(loc_image_position_x == blockDim.x-1)
     for(int offset_x = 4+1; offset_x<8+1; offset_x++)
       {
	     int ima_cd = min(max(ab_image_position_x-4+offset_x, 0), (numCols - 1));
    ...
    if(blockIdx.x < numBlock - 1 && blockIdx.y < numBlock - 1 ){
      if(loc_image_position_x == 0 && loc_image_position_y == 0)          
		  for(int offset_y = 0; offset_y < 4; offset_y++)
			for(int offset_x = 0; offset_x < 4; offset_x++)
          {            
            int ima_c1 = min(max(ab_image_position_x-4+offset_x, 0), (numCols - 1));
            int ima_r1 = min(max(ab_image_position_y-4+offset_y, 0), (numRows - 1));
         
            temp[(loc_image_position_y+offset_y)*(blockDim.x+8)+ loc_image_position_x+offset_x] = 
    ...
    }else
    {
	  //if(loc_image_position_x == 0 && loc_image_position_y == 0)          
		 for(int offset_y = 0; offset_y < 4; offset_y++)
		   for(int offset_x = 0; offset_x < 4; offset_x++)
         {
            int ima_c1_ = min(max(ab_image_position_x-4+offset_x, 0), (numCols - 1));
            int ima_r1_ = min(max(ab_image_position_y-4+offset_y, 0), (numRows - 1));
    ...
    if(blockIdx.x < numBlock - 1 && blockIdx.y < numBlock - 1 )
    {
       if(loc_image_position_x == blockDim.x - 1 && loc_image_position_y == 0)
	     for(int offset_y = 0; offset_y < 4; offset_y++)
		   for(int offset_x = 4+1; offset_x < 8+1; offset_x++)
         {      
            int ima_c2 = min(max(ab_image_position_x+offset_x-4, 0), (numCols - 1));
            int ima_r2 = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
         
            temp[(loc_image_position_y+offset_y)*(blockDim.x+8)+loc_image_position_x+offset_x] = 
            inputChannel[(ima_r2)*numCols+ima_c2];
         }
     }else if (blockIdx.x == numBlock - 1 && blockIdx.y == numBlock - 1) {
	  //if(loc_image_position_x == blockDim.x - 1 && loc_image_position_y == 0)
	     for(int offset_y = 0; offset_y < 4; offset_y++)
		   for(int offset_x = 4+1; offset_x < 8+1+BLOCK_SIZE-blockDim.x; offset_x++)
         ...
         {
            int ima_c2_ = min(max(ab_image_position_x+offset_x-4, 0), (numCols - 1));
            int ima_r2_ = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
    ...
    //left bottom
    if(blockIdx.x < numBlock - 1 && blockIdx.y < numBlock - 1 ){
      if(loc_image_position_x == 0 && loc_image_position_y == blockDim.y - 1)
	     for(int offset_y = 4+1; offset_y < 8+1; offset_y++)
		   for(int offset_x = 0; offset_x < 4; offset_x++)
         {
            int ima_c3_ = min(max(ab_image_position_x+offset_x-4, 0), (numCols - 1));
            int ima_r3_ = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
	  ...
	  //if(loc_image_position_x == 0 && loc_image_position_y == blockDim.y - 1)
	     for(int offset_y = 4+1; offset_y < 8+1+BLOCK_SIZE-blockDim.y; offset_y++)
		   for(int offset_x = 0; offset_x < 4; offset_x++)
         {
            int ima_c3 = min(max(ab_image_position_x+offset_x-4, 0), (numCols - 1));
            int ima_r3 = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
    ...
    //right bottom
    if(blockIdx.x < numBlock - 1 && blockIdx.y < numBlock - 1 ){
      if(loc_image_position_x == blockDim.x - 1 && loc_image_position_y == blockDim.y - 1)
	    for(int offset_y = 4+1; offset_y < 8+1; offset_y++)
		   for(int offset_x = 4+1; offset_x < 8+1; offset_x++)
         {
            int ima_c4 = min(max(ab_image_position_x+offset_x-4, 0), (numCols - 1));
            int ima_r4 = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
    ...
    {
	  //The code below has to be commented
      //if(loc_image_position_x == blockDim.x - 1 && loc_image_position_y == blockDim.y - 1)
	    for(int offset_y = 4+1; offset_y < 8+1; offset_y++)
		   for(int offset_x = 4+1; offset_x < 8+1; offset_x++)
        {
            int ima_c4_ = min(max(ab_image_position_x+offset_x-4, 0), (numCols - 1));
            int ima_r4_ = min(max(ab_image_position_y+offset_y-4, 0), (numRows - 1));
   ...
   // for(int offset_y = 0; offset_y < 9; offset_y++)
     // for(int offset_x = 0; offset_x < 9; offset_x++)
      // filter_tmp[(offset_y*9+offset_x)] = filter[(offset_y*9+offset_x)];
    ...
    { 
        ...
        tempOutRed += (float)temp[(loc_image_position_y+4+filter_y)*(blockDim.x+8)
                              +loc_image_position_x+4+filter_x].x 
                  * myFilter[(filter_y+filterWidth/2)*filterWidth+filter_x+filterWidth/2];
        tempOutGreen += (float)temp[(loc_image_position_y+4+filter_y)*(blockDim.x+8)
                              +loc_image_position_x+4+filter_x].y 
                  * myFilter[(filter_y+filterWidth/2)*filterWidth+filter_x+filterWidth/2];
        tempOutBlue += (float)temp[(loc_image_position_y+4+filter_y)*(blockDim.x+8)
                              +loc_image_position_x+4+filter_x].z 
                  * myFilter[(filter_y+filterWidth/2)*filterWidth+filter_x+filterWidth/2];
  ...
  else
  {
       uchar4 outPixel = make_uchar4(static_cast<unsigned char>(tempOutRed),
                                 ...
                                 255);
     outputChannel[ab_image_position_y*numCols + ab_image_position_x] =outPixel;
